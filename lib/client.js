...
// Both persist to the `dsh-better-retry` settings namespace through the Host
// half's same-origin route /dsh-better-retry/config (the settings wire API
// only serves apiproxy-allowlisted namespaces); every slider release saves
// immediately.
...
// ===== settings access via this package's own host route =====
// The web settings wire API (settings.describe/mutate) only serves namespaces
// allowlisted by dsh-host-apiproxy, and this package's namespace is not one
// of them, so the sliders read/write through the Host half's same-origin
// route /dsh-better-retry/config instead.
...
function writeField(field, value) {
  return fetch(CONFIG_URL, {
    ...
    return response.json(); // new resolved section
  });
}
...
exports.apply = function apply(ctx) {
  ...
  ctx.effect(function () {
    ...
    return locale.register('dsh-better-retry', {
      zh: {
        'retry.title': '失败重试次数',
